...
Modified the _predict_sentence function.
'''
...
class RelationExtractor(Model):
    ...
    def forward(self,  # type: ignore
        ...
        prediction_dict = self.predict(top_spans,
                                        relation_scores,
                                        num_spans_to_keep,
                                        metadata)

        output_dict = {"predictions": prediction_dict}
    ...
    # modifying the predict method
    def predict(self, top_spans, relation_scores, num_spans_to_keep, metadata):
        softmax_scores = F.softmax(relation_scores, dim=-1)  # Compute softmax to get probabilities
        preds_dict = []
        zipped = zip(top_spans, softmax_scores, num_spans_to_keep, metadata)  # Pass softmax_scores here
    
        for top_spans_sent, softmax_scores_sent, num_spans_sent, sentence in zipped:
            pred_dict_sent = self._predict_sentence(
                top_spans_sent, softmax_scores_sent, num_spans_sent, sentence)  # Use softmax_scores in prediction
            preds_dict.append(pred_dict_sent)
    
        return preds_dict

    # modified the _predict_sentence function
    def _predict_sentence(self, top_spans, softmax_scores, num_spans_to_keep, sentence):
        ...
        for i in range(keep):
            for j in range(keep):
                if i != j:
                    # Extract the maximum probability and its corresponding label
                    prob, label_idx = softmax_scores[i, j].max(dim=0)
                    if label_idx.item() > 0:  # Ignore the null relation (index 0)
                        label_name = self.vocab.get_token_from_index(label_idx.item() - 1, self._active_namespace)
                        span_pair = (top_spans[i], top_spans[j])
                        predictions_sent.append({
                            "span1": top_spans[i],
                            "span2": top_spans[j],
                            "label": label_name,
                            "probability": prob
        ...
        # Convert it to the desired output format if necessary
