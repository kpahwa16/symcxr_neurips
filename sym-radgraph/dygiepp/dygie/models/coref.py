'''
Modified the predict_labels_doc function
'''
import logging
import math
...
class CorefResolver(Model):
    ...
    def predict_labels_doc(self, output_dict):
        ...
        # Applying softmax to get probabilities for each antecedent choice, including the null choice.
        coreference_probs = F.softmax(coreference_scores, dim=2)
        
        _, predicted_antecedents = coreference_scores.max(2)
        predicted_antecedents -= 1  # Adjust for the null label being -1.
        
        # Extracting probabilities for the predicted antecedents.
        predicted_antecedents_probs = coreference_probs.max(2).values
        
        output_dict["predicted_antecedents"] = predicted_antecedents
        output_dict["predicted_antecedents_probs"] = predicted_antecedents_probs
        ...
        if coref_labels is not None:
            ...
            self._conll_coref_scores(top_spans, valid_antecedent_indices.unsqueeze(0), predicted_antecedents, evaluation_metadata)
