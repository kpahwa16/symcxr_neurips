'''
Modified the _decode_trigger function
Modified the _decode_arguments function
Modified the predict function
Modified the _assemble_predictions function
'''
...
class EventExtractor(Model):
    ...
    # modified the _decode_trigger function to return probability.
    def _decode_trigger(self, output):
        trigger_scores = output["trigger_scores"]
        softmax_scores = F.softmax(trigger_scores, dim=2)
        predicted_scores_softmax, predicted_triggers = softmax_scores.max(dim=2)
        trigger_dict = {}
        for i in range(output["sentence_lengths"]):
            trig_label = predicted_triggers[0, i].item()  # Assuming single sentence
            if trig_label > 0:
                predicted_label = self.vocab.get_token_from_index(trig_label, namespace=self._active_namespaces["trigger"])
                trigger_dict[i] = {
                    "span_start": i,  # For triggers, start and end are often the same
                    "span_end": i,
                    "predicted_label": predicted_label,
                    "probability": predicted_scores_softmax[0, i].item(),
                    "all_probabilities": softmax_scores[0, i].tolist()  # Includes probabilities for all labels
                }
        return trigger_dict

    # modified the _decode_arguments function to return probabilities
    def _decode_arguments(self, output, decoded_trig):
        argument_dict = {}
        argument_scores = output["argument_scores"]
        softmax_scores = F.softmax(argument_scores, dim=3)
        predicted_scores_softmax, predicted_arguments = softmax_scores.max(dim=3)
        predicted_arguments -= 1  # Adjust for null label being -1
    
        top_argument_spans = output["top_argument_spans"]  # Assuming this contains the span start and end indices
    
        for i, j in itertools.product(range(output["num_triggers_kept"]), range(output["num_argument_spans_kept"])):
            trig_ix = output["top_trigger_indices"][0, i].item()  # Assuming single sentence
            arg_span_start, arg_span_end = top_argument_spans[0, j].tolist()  # Extract start and end indices
            arg_label = predicted_arguments[0, i, j].item()
            if arg_label >= 0 and trig_ix in decoded_trig:
                label_name = self.vocab.get_token_from_index(arg_label, namespace=self._active_namespaces["argument"])
                argument_dict[(trig_ix, (arg_span_start, arg_span_end))] = {
                    "span_start": arg_span_start,
                    "span_end": arg_span_end,
                    "predicted_label": label_name,
                    "probability": predicted_scores_softmax[0, i, j].item(),
                    "all_probabilities": softmax_scores[0, i, j].tolist()  # Includes probabilities for all labels
                }
    ...
    def _assemble_predictions(self, trigger_dict, argument_dict, sentence):
        events = []
        for trigger_ix, trigger_info in trigger_dict.items():
            # Extract trigger event details
            trigger_event = {
                "trigger": {
                    "index": trigger_ix,
                    "start": trigger_info["span_start"],
                    "end": trigger_info["span_end"],
                    "label": trigger_info["predicted_label"],
                    "probability": trigger_info["probability"],
                    "all_probabilities": trigger_info["all_probabilities"],
                },
                "arguments": []
            }
    
            # Collect arguments related to the trigger
            for ((trig_ix, span), arg_info) in argument_dict.items():
                if trig_ix == trigger_ix:
                    argument_event = {
                        "span_start": arg_info["span_start"],
                        "span_end": arg_info["span_end"],
                        "label": arg_info["predicted_label"],
                        "probability": arg_info["probability"],
                        "all_probabilities": arg_info["all_probabilities"],
                    }
                    trigger_event["arguments"].append(argument_event)
