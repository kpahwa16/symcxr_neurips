...
class Sentence:
    ...
    def to_json(self):
        ...
        if self.predicted_ner is not None:
            if type(self.predicted_ner[0]) == dict:
                res["predicted_ner"] = self.predicted_ner
            else:
        ...
        if self.predicted_relations is not None:
            if type(self.predicted_relations[0]) == dict:
                res['predicted_relations'] = self.predicted_relations
            else:
