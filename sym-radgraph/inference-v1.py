...
def run_inference(model_path, data_path, cuda):
    ...
    for file_name, datapoint in dataset_reader.dataset.items():
        result = predictor.predict_instance(
                dataset_reader.text_to_instance(preprocess_report(file_name, datapoint['text']))
                )
        break
...
def run(model_path, data_path, out_path, cuda):
    ...
    run_inference(model_path, data_path, cuda)
...
if __name__ == '__main__':
    ...
    data_dir = os.path.abspath(os.path.join(os.path.abspath(__file__), '../../data/medical_imaging'))
    ...
    run(checkpoint_path, data_path, output_path, cuda_device)
